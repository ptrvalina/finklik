...
export default function AppModal({ title, onClose, children, wide, extraWide, footer }: AppModalProps) {
  return (
    ...
    >
      <div
        className={`flex h-[100dvh] max-h-[100dvh] w-full flex-col bg-surface shadow-2xl sm:h-auto sm:max-h-[min(90vh,900px)] sm:rounded-3xl sm:border sm:border-outline/90 sm:shadow-lift dark:sm:border-outline/45 ${
      ...
      >
        <div className="flex flex-shrink-0 items-center justify-between gap-3 border-b border-outline/80 px-4 py-4 dark:border-outline/35 sm:px-6 sm:py-5">
          ...
          <button
            type="button"
            onClick={onClose}
            className="tap-highlight-none flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl border border-outline/75 bg-surface-container-low text-on-surface-variant hover:border-primary/30 hover:bg-surface-container-high hover:text-on-surface dark:border-outline/45 dark:text-on-surface-variant"
            aria-label="Закрыть"
          >
        ...
        {footer != null && (
          <div className="flex-shrink-0 border-t border-outline/55 bg-surface-container-low/80 px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom,0px))] dark:border-outline/35 sm:rounded-b-2xl sm:px-6 sm:pb-4">
            {footer}
          </div>
        )}
      </div>
