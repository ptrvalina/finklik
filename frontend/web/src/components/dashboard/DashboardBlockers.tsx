...
/** Блок 3 главной: что мешает двигаться дальше (plain language). */
export default function DashboardBlockers() {
  ...
  const blockers = calm?.readiness?.blockers ?? []
  for (const b of blockers.slice(0, 3)) {
    rows.push({
      id: `rb-${b.code ?? b.label}`,
      label: b.label,
      to: '/reports',
    })
  }
  ...
  if (ocr > 0 && rows.length < 5) {
    rows.push({
      id: 'ocr',
      label: ocr === 1 ? 'Проверить 1 документ после OCR' : `Проверить ${ocr} документов после OCR`,
      to: '/scan',
    })
  }
  ...
  if (rows.length === 0) {
    return (
      <section className="glass-card rounded-2xl p-5 sm:p-6">
        <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-on-surface-variant">Препятствия</p>
