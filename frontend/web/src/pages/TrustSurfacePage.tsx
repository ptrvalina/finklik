    ...
    >
      ...
      {isError && (
        <CalmErrorState
          ...
          fallbackMessage="Не удалось загрузить поверхность доверия."
          onRetry={() => void refetch()}
        />
      )}
      {!isLoading && !isError && data && (
        <div className="fc-section-stack space-y-6">
          <section className="rounded-2xl border border-outline/35 bg-surface/90 p-5">
            <p className="text-[10px] font-bold uppercase tracking-wide text-primary">Спокойные индикаторы</p>
          ...
          <section className="rounded-2xl border border-outline/35 bg-surface/90 p-5">
            <p className="text-[10px] font-bold uppercase tracking-wide text-primary">Фоновые процессы</p>
            <ul className="mt-3 space-y-3 text-sm">
              {data.background_jobs.map((j) => (
                <li key={j.id} className="text-on-surface-variant">
                  <span className="font-medium text-on-surface">{j.domain}</span> — {j.status_plain}
          ...
          {data.state_consistency && (
            <p className="rounded-2xl border border-outline/30 bg-surface-container-low/50 px-4 py-3 text-sm text-on-surface-variant">
              <span className="font-medium text-on-surface">Согласованность: </span>
          ...
          <section className="rounded-2xl border border-outline/25 bg-surface-container-low/40 px-4 py-4 text-sm text-on-surface-variant">
            <p className="font-bold uppercase tracking-wide text-primary text-xs">Безопасные действия</p>
