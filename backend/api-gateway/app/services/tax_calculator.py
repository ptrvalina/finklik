...
НДС: ставки 0%, 10%, 20% по видам операций; в расчёте по умолчанию — ставка из конфига (vat_rate)
...
VAT_RATE = Decimal("0.20")             # базовая ставка для расчёта в приложении (фактические 0/10/20% — по операциям)
...
def calculate_vat(
    ...
    Фактические ставки НДС могут быть 0%, 10% или 20%; здесь передаётся выбранная для модели.
